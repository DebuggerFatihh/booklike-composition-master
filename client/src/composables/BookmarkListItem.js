...
const appAxios=inject("appAxios");
    ...
    const likeItem=()=> {
        appAxios({
          url: alreadyLiked.value ? `/UserLike/${likedItem.value.id}` : "/UserLike",
  ...
  const bookmarkItem=()=>{
        appAxios({
          url: alreadyBookmarked.value ? `/UserBookmark/${bookmarkedItem.value.id}` : "/UserBookmark",
      ...
      const _getCurrentUser=computed(()=>store.getters._getCurrentUser.value)
      const _userLikes=computed(()=>store.getters._userLikes)
      const _userBookmarks=computed(()=>store.getters._userBookmarks)
   
      return {likeItem,bookmarkItem,categoryName,userName,alreadyLiked,alreadyBookmarked
            ,bookmarkedItem,likedItem,_getCurrentUser,_userLikes,_userBookmarks}
}